...
- **LaTeX Parser Design** - new IMathItemBuilder/IParserAdapter interface
- **LaTeX Parser: Tokenizer** - input->tokens \
- **LaTeX Parser: Grouping/nested token subsets**
- **LaTeX Parser: Most basic commands** - Text/Math mode multiline processing, no environments**
- **LaTeX Parser - Text mode accents** - \'{a}, \`{a},...
- **MathMode Environments** - matrix,array,cases,...
+
### 🚧 In Progress
- **Overlay/Decor/Annot commands** - in both Text/Math modes: \boxed, \underbar,\underline, \cancel \sout, etc.
- **siunitx commands** - \num{#.#|#,###,...|#.#e{-}#},\angle{#|#;#|#;#;#}

### 📋 Planned Features
- **MathMode Environments** - equation, align, ...**
- **MathBox Control** - Complete control with zooming and selection/copy/paste support
...
- `IMathItemBuilder` - Interface for abstract MathItem builders
- `IParserAdapter` - Helepr interface for processing LaTeX "builder" commands
